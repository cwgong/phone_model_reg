...
from struct_zgc_info import Struct_Info
import sys
import shutil
import json
...
class DeviceRegTool(object):
    ...
    def _device_reg_clean_v3(self,ori_file_jd,ori_file_tmall,output_file, phone_element, json_file):
        '''
    ...
    def _device_reg_clean_v4(self, ori_file_jd, ori_file_tmall, output_file, phone_element, json_file):
        '''
        ...
        for product_info_dict in data_list_all:
            ...
            #
            # if tmp_dict == None:
            #     brand_result = self.brand_recognition(sku_name_tmp)
            #     if brand_result in self.confound_sku_dict:
            #         if brand_result == self.brand_name or brand_result == self.confound_sku_dict[self.brand_name][0]:
            #             err_lst.append(product_info_dict)
            #         else:
            #             device_loading_obj_ext,device_list_ext,brand_id_ext,phone_model_dict = self.clean_model_loading(brand_result,phone_element, json_file)
            #             tmp_dict = self.model_clean_ext(sku_name_tmp, device_list_ext)
            #             if tmp_dict == None:
            #                 err_lst.append(product_info_dict)
            #             else:
            #                 flag_reg = 3
            #     elif brand_result not in self.confound_sku_dict:
            #         if brand_result == self.brand_name:
            #             err_lst.append(product_info_dict)
            #         else:
            #             device_loading_obj_ext,device_list_ext,brand_id_ext,phone_model_dict = self.clean_model_loading(brand_result, phone_element, json_file)
            #             tmp_dict = self.model_clean_ext(sku_name_tmp, device_list_ext)
        ...
        # print(len(err_lst))
        with io.open(output_file + "1212_"+"clean_result.txt", "a", encoding="utf-8") as f2:
            f2.write("\n".join(r_lst))
            f2.flush()
        with io.open(output_file + "1212_"+"error_result.txt","a",encoding="utf-8") as f3:
            ...
            f3.flush()
        with io.open(output_file + "1212_"+"error_save_result.txt","a",encoding="utf-8") as f4:
            for item_dict in err_lst:
    ...
    # def _device_reg_clean_demo(self, ori_file_jd, ori_file_tmall, output_file, phone_element, json_file):
    #     '''
    #     The function is updated in 2020.12.04 by gcw.
    #     :param ori_file:
    #     :param output_file:
    ...
    #                 continue
        ...
        #                  (pid, brand_id_std, brand_name_std, phone_model, data_source, \
        #                   zgc_info_dict['camera_num'], zgc_info_dict['main_screen_resolution'],
        #                   zgc_info_dict['main_screen_material'], zgc_info_dict['5g'], zgc_info_dict['rear_camera_max'], \
        #                   zgc_info_dict['rear_camera_info'], zgc_info_dict['rear_camera_num'],
        #                   zgc_info_dict['front_facing_camera_max'], zgc_info_dict['front_facing_camera_info'], \
        #                   zgc_info_dict['release_time']))
        #
        #     if flag_reg == 1:
        #         brand_list = self.brand_seg_dict[self.brand_name]       #清洗的过程中添加了品牌名称，对应到型号中时需要去掉品牌名称进行型号对应
...
# def _device_reg_clean_all(ori_file_jd, ori_file_tmall, output_file, phone_element, json_file):
#     '''
#     The function is updated in 2020.12.04 by gcw.
#     :param ori_file:
#     :param output_file:
